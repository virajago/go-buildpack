...
func (gf *Finalizer) SetMainPackageName() error {
	...
	case "go_nativevendoring":
		...
		if gf.MainPackageName == "" {
			gf.Log.Error(warningMessages.NoGOPACKAGENAMEerror())
...
func (gf *Finalizer) HandleVendorExperiment() error {
	...
	if !go16 {
		gf.Log.Error(warningMessages.UnsupportedGO15VENDOREXPERIMENTerror())
...
func (gf *Finalizer) SetInstallPackages() error {
	...
	if gf.VendorTool == "godep" {
		...
		if gf.Godep.WorkspaceExists && vendorDirExists {
			gf.Log.Warning(warningMessages.GodepsWorkspaceWarning())
		}
		...
		if len(packages) != 0 {
			gf.Log.Warning(warningMessages.PackageSpecOverride(packages))
		...
		if !gf.VendorExperiment && gf.VendorTool == "go_nativevendoring" {
			gf.Log.Error(warningMessages.MustUseVendorError())
